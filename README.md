# COMP 2230-02: Data Structures, Algorithm Analysis, and Program Design

**Term:** Winter 2026

**Instructor:** Shivani Tyagi

**Class Venue:** OM 1350

**Office:** 406B Clock Tower

**Office Hours:** By appointment

---

## 📌 Course Overview

This repository serves as the **central hub for all course materials** for **COMP 2230-02**. Since Moodle access is unavailable, all lecture content, lab materials, worksheets, code examples, and announcements will be maintained here on GitHub.

Students are expected to regularly check this repository for updates.

---

## 🕒 Class Schedule

* **Lectures:** Monday, Wednesday, Thursday — 6:00 PM to 6:50 PM
* **Seminar:** Wednesday — 7:00 PM to 7:50 PM

### Recognized Holidays (No Classes)

* Feb 16 – Family Day
* Apr 3 – Good Friday
* Apr 6 – Easter Monday

### Important Dates

* **Midterm Break:** Feb 17 – Feb 20
* **Final Exam Window:** Apr 16 – Apr 29

---

## 🎯 Course Description

This course introduces fundamental methods of representing and organizing data in Computing Science. Students study, implement, and analyze core data structures such as lists, stacks, queues, trees, and graphs. Emphasis is placed on algorithm design, efficiency analysis (time and space complexity), and trade-offs between data structures to support effective program design and problem-solving.

---

## ✅ Learning Outcomes

By the end of this course, students will be able to:

* Identify problems solvable using common data structures
* Implement basic data structures and operations in **Java**
* Understand and implement sorting algorithms:

  * Bubble Sort
  * Selection Sort
  * Insertion Sort
  * Merge Sort
  * Quick Sort
* Apply searching techniques:

  * Linear Search
  * Binary Search
  * Hash-based Search
* Analyze algorithm efficiency using mathematical techniques
* Develop middle-sized programs using appropriate programming practices

---

## 📚 Prerequisites

* Grade of **C or better** in:

  * COMP 1230 or COMP 1231
  * MATH 1700 or MATH 1701

---

## 🗂️ Repository Structure

```
COMP-2230-Winter-2026/
│
├── README.md                     # Course overview and instructions
├── outline/                     # Official course outline and policies
│   └── COMP2230_course_outline.pdf
│
├── java_101_introduction/        # Java programming basics and fundamentals
│   ├── README.md                 # Introduction overview
│   ├── basic_syntax/             # Variables, data types, operators
│   ├── control_flow/             # If-else, loops
│   ├── methods/                  # Functions and procedures
│   ├── classes_objects/          # OOP basics
│   ├── arrays/                   # Fixed-size data structures
│   └── input_output/             # Console I/O operations
│
├── lectures/                     # Weekly lecture slides & notes
│   ├── week01_algorithm_analysis/
│   ├── week02_stacks_generics/
│   ├── week03_linked_structures/
│   ├── week04_queues/
│   ├── week05_lists/
│   ├── week07_searching_sorting_intro/
│   ├── week08_sorting_algorithms/
│   ├── week09_trees_bst/
│   ├── week10_heaps_priority_queues/
│   ├── week12_hashing/
│   ├── week13_sets_maps_graphs/
│   └── review_final_exam/
│
├── labs/                         # Weekly lab exercises
│   ├── lab01_big_o_analysis/
│   ├── lab02_stacks/
│   ├── lab03_linked_lists/
│   ├── lab04_queues/
│   ├── lab05_lists_iterators/
│   ├── lab06_searching/
│   ├── lab07_sorting/
│   ├── lab08_trees/
│   ├── lab09_heaps_priority_queues/
│   ├── lab10_hash_tables/
│   └── lab11_graphs/
│
├── worksheets/                   # Weekly worksheets (PDF / Markdown)
│   ├── worksheet01_week01/
│   ├── worksheet02_week02/
│   ├── worksheet03_week03/
│   ├── worksheet04_week05/
│   ├── worksheet05_week07/
│   ├── worksheet06_week08/
│   ├── worksheet07_week09/
│   ├── worksheet08_week10/
│   ├── worksheet09_week12/
│   └── worksheet10_week13/
│
├── code-examples/                # Reference Java implementations
│   ├── stacks/
│   ├── queues/
│   ├── linkedlists/
│   ├── sorting/
│   ├── searching/
│   ├── trees/
│   ├── heaps/
│   ├── hashing/
│   └── graphs/
│
├── exams/                        # Exam info and review material
│   ├── midterm1_review/
│   ├── midterm2_review/
│   └── final_exam_review/
│
├── industry-usecase/             # Industry case study materials
│   └── data_structures_case_study.md
│
├── tools/                        # Setup guides
│   ├── github_codespaces_setup.md
│   └── java_environment_setup.md
│
└── announcements/                # Course announcements and updates
```

---

## 🧪 Labs

Lab sessions reinforce lecture concepts through hands-on Java programming. You will:

* Implement data structures
* Analyze algorithm efficiency
* Compare different approaches
* Experiment and debug code

Labs are **practice-focused** and directly aligned with weekly lecture topics.

---

## 📝 Assessments & Grading

| Assessment                  | Weight |
| --------------------------- | ------ |
| Attendance                  | 5%     |
| Weekly Worksheets (10 × 2%) | 20%    |
| Midterm 1                   | 20%    |
| Midterm 2                   | 20%    |
| Final Exam                  | 35%    |

---

## 📆 Worksheet & Exam Timeline

* Worksheets released **every Monday** (except midterm weeks & Week 1)
* Due **Sunday 11:59 PM** of the same week
* Delivered and submitted as instructed in class

Midterms and final exams are **closed-book**, timed, and conducted via Moodle.

---

## 💻 Technology Used

* **Java** programming language
* **GitHub Codespaces** (Java Interpreter)
* **Moodle** (exams only)

Setup instructions are provided in the `tools/` directory.

---

## 💬 Communication Guidelines

* **General questions (lectures, assignments, exams):** Use Moodle discussion forum
* **Personal or individual concerns:** Email the instructor

**Response Time:** 9 AM – 5 PM (Mon–Fri). Messages outside working hours will be addressed the next business day.

---

## 📋 Attendance & Academic Policies

* Minimum **75% attendance** required to pass the course
* Attendance will be recorded via Moodle

### Late Assignments

* 25% penalty per day (max 75%)
* No submissions accepted after 3 days

### Academic Concessions

* Notify instructor **at least 24 hours before** deadline
* Documentation may be required

### Accessibility

Students registered with Accessibility Services must submit their **Letter of Accommodation** early in the term.

---

## 🤖 Generative AI Policy

Students must follow TRU’s guidelines on the use of AI tools (e.g., ChatGPT, Copilot):
[https://libguides.tru.ca/artificialintelligence](https://libguides.tru.ca/artificialintelligence)

---

## 📎 Final Notes

This GitHub repository is your **primary learning platform** for COMP 2230. Make sure to:

* Pull updates regularly
* Read announcements
* Review code examples
* Practice consistently

Welcome to the course, and happy coding! 🚀
